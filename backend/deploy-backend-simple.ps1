...
if (-not (Test-Path $backendDir)) {
    Write-Host "Error: Backend directory does not exist at $backendDir"
    exit 1
}
...
try {
    ...
    if (-not $whoami) {
        Write-Host "Please login first: railway login"
        exit 1
    } else {
...
if ($backendUrl) {
    ...
    Write-Host "Live Backend URL: $backendUrl"
} else {
    Write-Host "Could not retrieve deployment URL. Check Railway dashboard:"
    Write-Host "https://railway.com/project/b44ace12-b8f4-4eb1-b971-06eed436c7c7"
}
